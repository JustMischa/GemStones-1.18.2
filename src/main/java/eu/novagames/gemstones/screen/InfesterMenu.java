...
import eu.novagames.gemstones.screen.slot.ModResultSlot;
...
public class InfesterMenu extends AbstractContainerMenu {

    public final InfesterEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public InfesterMenu(int pContainerId, Inventory inventory, FriendlyByteBuf extraData) {
        this(pContainerId, inventory, inventory.player.level.getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(2));
    }

    public InfesterMenu(int pContainerId, Inventory inventory, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.INFESTER_MENU.get(), pContainerId);
        checkContainerSize(inventory, 3);
        blockEntity = ((InfesterEntity) entity);
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.INFESTER.get());
    }
