...
public class FlammableRotatedPillarBlock extends RotatedPillarBlock {
    ...
    public BlockState getToolModifiedState(BlockState state, Level world, BlockPos pos, Player player,
        ...
        if(stack.getItem() instanceof AxeItem) {
            if(state.is(ModBlocks.TENEBRIS_LOG.get())) {
                return ModBlocks.STRIPPED_TENEBRIS_LOG.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS));
            }
            if(state.is(ModBlocks.TENEBRIS_WOOD.get())) {
                return ModBlocks.STRIPPED_TENEBRIS_WOOD.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS));
            }
